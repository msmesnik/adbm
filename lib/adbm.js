...
function adbm ({ db, dbName, adapter, metadata = '_adbm', directory = 'migrations', logger = defaultLogger }) {
  if (!db) {
  ...
  return async ({ direction = UP, exclude = [ ] } = { }) => {
    logger.debug('○ Initializing database migrations, direction is %s.', direction)

    await adapter.init({ db, dbName, metadata, directory, logger })
...
async function performMigrations ({ migrations, direction, db, metadata, registerSuccess, logger = defaultLogger }) {
  ...
  return Promise.mapSeries(
    migrateUp ? migrations : [ ...migrations ].reverse(), // avoid mutating passed array
    async ({ [direction]: fn, id }) => {
